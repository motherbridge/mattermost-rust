...
/// struct for typed errors of method [`get_roles_by_names`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetRolesByNamesError {
    Status400(crate::models::AppError),
    Status401(crate::models::AppError),
    Status404(crate::models::AppError),
    UnknownValue(serde_json::Value),
...
pub async fn get_role_by_name(configuration: &configuration::Configuration, role_name: &str) -> Result<crate::models::Role, Error<GetRoleByNameError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/roles/name/{role_name}", local_var_configuration.base_path, role_name=crate::apis::urlencode(role_name));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetRoleByNameError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn get_roles_by_names(configuration: &configuration::Configuration, request_body: Vec<String>) -> Result<Vec<crate::models::Role>, Error<GetRolesByNamesError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/roles/names", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetRolesByNamesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn patch_role(configuration: &configuration::Configuration, role_id: &str, patch_role_request: crate::models::PatchRoleRequest) -> Result<crate::models::Role, Error<PatchRoleError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/roles/{role_id}/patch", local_var_configuration.base_path, role_id=crate::apis::urlencode(role_id));
    ...
    local_var_req_builder = local_var_req_builder.json(&patch_role_request);
